...
# Durango PostgreSQL Database Setup Script
# Creates PostgreSQL user and database for Durango PG service
# Runs migrations and seeding

echo "=================================================="
echo "Durango PG - PostgreSQL Database Setup"
echo "=================================================="
echo ""
...
DB_NAME=${DB_NAME:-durango}
DB_USERNAME=${DB_USERNAME:-durango}
DB_PASSWORD=${DB_PASSWORD:-durango}
...
# Check if durango-pg container exists
echo "Step 2: Checking if Durango PG is available..."
if docker compose ps durango-pg 2>/dev/null | grep -q "durango-pg"; then
    if docker compose ps durango-pg | grep -q "Up"; then
        echo -e "${GREEN}✓ Durango PG container is running${NC}"
        DURANGO_AVAILABLE=true
    else
        echo -e "${YELLOW}⚠ Durango PG container exists but is not running${NC}"
        DURANGO_AVAILABLE=false
    fi
else
    echo -e "${YELLOW}⚠ Durango PG service is not configured${NC}"
    DURANGO_AVAILABLE=false
fi
echo ""
...
# Run migrations and seeders if Durango is available
if [ "$DURANGO_AVAILABLE" = true ]; then
    echo "Step 7: Running database migrations..."
    if docker compose exec durango-pg php bin/cake.php migrations migrate 2>&1 | tee /tmp/migrations.log; then
    ...
    if docker compose exec durango-pg php bin/cake.php migrations seed 2>&1 | tee /tmp/seeders.log; then
    ...
    echo -e "${YELLOW}⚠ Skipping migrations/seeders (Durango PG not running)${NC}"
    echo "To run migrations later:"
    echo "  docker compose exec durango-pg php bin/cake.php migrations migrate"
    echo "  docker compose exec durango-pg php bin/cake.php migrations seed"
    echo ""
fi

echo "=================================================="
echo "Durango PG Setup Complete!"
echo "=================================================="
echo ""
