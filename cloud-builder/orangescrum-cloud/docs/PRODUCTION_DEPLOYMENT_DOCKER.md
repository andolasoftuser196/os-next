...
sudo chown $USER:$USER /opt/orangescrum
cd /opt/orangescrum
```
...
scp orangescrum-frankenphp-v*.tar.gz user@server:/opt/orangescrum/
...
SESSION_COOKIE_DOMAIN=.yourdomain.com  # For subdomain sharing
...
GOOGLE_OAUTH_REDIRECT_URI=https://app.yourdomain.com/oauth/google/callback
...
psql -h your-postgres-server.example.com -U postgres
...
*/5 * * * * curl -fsS -m 10 --retry 3 https://app.yourdomain.com/home/healthcheck > /dev/null || echo "OrangeScrum health check failed" | mail -s "Alert: OrangeScrum Down" admin@yourdomain.com
```
...
```bash
chmod +x /opt/orangescrum/backup-database.sh

# Add to cron (daily at 2 AM)
crontab -e
0 2 * * * /opt/orangescrum/backup-database.sh
```
...
```bash
cd /opt/orangescrum/orangescrum-frankenphp-v*/
