...
if [ -d "$EXTRACTED_APP/config" ]; then
    ...
    # Cache config (copy only the one matching CACHE_ENGINE env var)
    CACHE_ENGINE="${CACHE_ENGINE:-redis}"
    CACHE_ENGINE=$(echo "$CACHE_ENGINE" | tr '[:upper:]' '[:lower:]')
    if [[ ! "$CACHE_ENGINE" =~ ^(redis|file|memcached|auto)$ ]]; then
        echo "  [WARNING] Invalid CACHE_ENGINE '$CACHE_ENGINE', falling back to 'redis'"
        CACHE_ENGINE="redis"
    fi
    copy_config_file "$EXTRACTED_APP/config/cache_${CACHE_ENGINE}.example.php" "$EXTRACTED_APP/config/cache_${CACHE_ENGINE}.php" "cache_${CACHE_ENGINE}.php (active)"
    
    # Queue configs
    copy_config_file "$EXTRACTED_APP/config/queue.example.php" "$EXTRACTED_APP/config/queue.php" "queue.php"
    
    # Email configs (copy only the one matching EMAIL_TRANSPORT env var)
    EMAIL_TRANSPORT="${EMAIL_TRANSPORT:-sendgrid}"
    EMAIL_TRANSPORT=$(echo "$EMAIL_TRANSPORT" | tr '[:upper:]' '[:lower:]')
    if [[ ! "$EMAIL_TRANSPORT" =~ ^(smtp|sendgrid)$ ]]; then
        echo "  [WARNING] Invalid EMAIL_TRANSPORT '$EMAIL_TRANSPORT', falling back to 'sendgrid'"
        EMAIL_TRANSPORT="sendgrid"
    fi
    copy_config_file "$EXTRACTED_APP/config/${EMAIL_TRANSPORT}.example.php" "$EXTRACTED_APP/config/${EMAIL_TRANSPORT}.php" "${EMAIL_TRANSPORT}.php (active)"
