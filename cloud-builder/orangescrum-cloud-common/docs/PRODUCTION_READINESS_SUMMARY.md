...
|----------|--------|-------|
| **Security** | **CRITICAL FIXES REQUIRED** | Default passwords, validation missing |
| **Architecture** | **GOOD** | Stateless, externalized services |
| **Deployment** | **NEEDS IMPROVEMENT** | Missing production guides |
| **Monitoring** | **NEEDS SETUP** | No built-in monitoring |
| **Backup** | **NEEDS DOCUMENTATION** | Procedures not documented |
...
sudo mkdir -p /opt/application /var/log/orangescrum
...
- [ ] Health check returns HTTP 200: `curl https://app.<your-domain>/home/healthcheck`
...
- **Docker Mode:** Recommended for most users, simplified scaling
- **Native Mode:** Lower overhead, optimized for single-server deployments
