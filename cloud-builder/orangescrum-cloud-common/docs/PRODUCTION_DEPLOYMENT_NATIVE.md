...
sudo useradd -r -m -d /opt/application -s /bin/bash orangescrum
...
sudo mkdir -p /opt/application/logs
sudo mkdir -p /opt/application/backups
sudo mkdir -p /var/log/orangescrum

# Set permissions
sudo chown -R <app-user>:<app-group> /opt/application
sudo chown -R <app-user>:<app-group> /var/log/orangescrum
```
...
```bash
# Test connection
PGPASSWORD="your-password" psql -h <db-host> \
...
# Start application
ExecStart=/opt/application/bin/orangescrum php-server -r webroot -l 127.0.0.1:8080
...
ProtectHome=true
ReadWritePaths=/opt/application /var/log/orangescrum /tmp
...
ExecStart=/opt/application/queue-worker.sh start
...
sudo certbot certonly --standalone -d app.<your-domain> \
  --agree-tos --email admin@<your-domain>
...
*/30 * * * * cd /opt/application && EXTRACTED_APP=$(find /tmp -maxdepth 1 -name "frankenphp_*" -type d | head -1) && [ -n "$EXTRACTED_APP" ] && cd "$EXTRACTED_APP" && /opt/application/bin/orangescrum php-cli bin/cake.php recurring_task >> /var/log/orangescrum/cron.log 2>&1
```
...
HEALTH_URL="https://app.<your-domain>/home/healthcheck"
...
```bash
sudo chmod +x /opt/application/backup-database.sh
sudo chown <app-user>:<app-group> /opt/application/backup-database.sh

# Add to orangescrum user crontab (daily at 2 AM)
sudo su - orangescrum
crontab -e
0 2 * * * /opt/application/backup-database.sh >> /var/log/orangescrum/backup.log 2>&1
```
...
openssl s_client -connect app.<your-domain>:443 -servername app.<your-domain> < /dev/null
...
sudo cp -a /opt/application /opt/application.backup-$(date +%Y%m%d)
...
# 4. Verify
curl https://app.<your-domain>/home/healthcheck
```
...
- **Documentation:** `/opt/application/docs/`
